        ...
        <div className="max-w-md text-center">
          ...
          <div className="flex flex-col items-center gap-2">
            <button
              ...
              disabled={refreshing}
              className="rounded-md border border-carbon-6 bg-carbon-1 px-4 py-1.5 text-xs text-text-2 hover:border-carbon-7 disabled:opacity-40 focus:outline-none focus-visible:ring-2 focus-visible:ring-amethyst-500"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-xs text-amethyst-400 underline decoration-amethyst-700 hover:text-amethyst-300"
            >
    ...
    <div className="relative flex min-h-0 flex-1">
      ...
      >
        ...
        {!loading && filteredMemories.length === 0 && memories.length > 0 && (
          <div className="pointer-events-none absolute inset-x-0 top-6 mx-auto w-fit rounded-md border border-carbon-4 bg-void/90 px-3 py-1.5 text-xs text-text-4">
      ...
      <div className="flex flex-col items-center gap-3">
        <div className="flex items-center gap-2">
          {[0, 1, 2].map((i) => (
            <span
              key={i}
              className="inline-block h-2 w-2 animate-pulse rounded-full bg-carbon-7"
              style={{ animationDelay: `${i * 150}ms` }}
            />
          ))}
        </div>
        <p className="text-xs text-text-5">Loading memories…</p>
      </div>
    </div>
  );
}
